...
You have been given a content idea and current research about the topic.
Analyse both and produce a detailed content PLAN that will guide platform-specific writers.
Do NOT write the posts yet.

Content idea:
{input_content}

Current research and context (from web search):
{search_context}

Your plan must cover:
1. Target audience (who will read this)
2. Core message (one sentence that every post must convey)
3. Key facts and insights from the research above that should be referenced
4. LinkedIn strategy  — tone, length, structure, professional angle, hashtag themes
5. X (Twitter) strategy — hook style, character constraints, viral angle, hashtag themes
6. Instagram strategy  — visual cue suggestions, storytelling angle, CTA type, hashtag themes
7. Key facts / phrases that must appear in all posts
8. What to AVOID (off-brand language, over-promotion, etc.)
